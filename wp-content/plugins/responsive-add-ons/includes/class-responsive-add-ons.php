...
class Responsive_Add_Ons {
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'rst_add_blocks_data' ) );
	...
	public function responsive_ready_sites_admin_enqueue_scripts( $hook = '' ) {
		...
		if ( 'toplevel_page_responsive_add_ons' === $hook && empty( $_GET['action'] ) ) {
	...
	public function responsive_ready_sites_admin_enqueue_styles( $hook = '' ) {
		if ( 'toplevel_page_responsive_add_ons' === $hook || 'responsive_page_responsive_add_ons_go_pro' === $hook ) {
	...
	public function rst_add_blocks_data() {
		$get_rst_blocks_data = get_site_option( 'rst-blocks-page-1', false );
		if ( ! $get_rst_blocks_data ) {
			require_once RESPONSIVE_ADDONS_DIR . 'includes/importers/batch-processing/class-responsive-ready-sites-batch-processing.php';
			$import_blocks_instance = new Responsive_Ready_Sites_Batch_Processing();
			$import_blocks_instance->process_blocks_batch();
		}
	}
}
