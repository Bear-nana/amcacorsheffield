...
if ( ! class_exists( 'Responsive_Ready_Sites_Batch_Processing' ) ) :
	...
	class Responsive_Ready_Sites_Batch_Processing {
		...
		public function get_total_rst_blocks_requests() {
			...
			$api_url = self::$api_url . 'get-rst-blocks-requests-count/?per_page=15';
		...
		public function initialize_ready_sites_templates_importer() {
			...
			if ( isset( $_GET['reset'] ) ) {
				...
				if ( 'toplevel_page_responsive_add_ons' === $current_screen->id ) {
					...
					$this->process_rst_blocks_import();
				}
			}
		}

		/**
		 * Process RST Blocks Import
		 *
		 * @since 2.9.1
		 *
		 * @return mixed Null if process is already started.
		 */
		public function process_rst_blocks_import() {
			$status = get_site_option( 'rst-blocks-batch-status' );

			if ( 'in-process' === $status ) {
				return;
			}

			$expired = get_transient( 'rst-blocks-import-check' );

			if ( false !== $expired ) {
				return;
			}

			set_transient( 'rst-blocks-import-check', 'true', WEEK_IN_SECONDS );

			update_site_option( 'rst-blocks-batch-status', 'in-process' );

			$this->process_blocks_batch();
		}

		/**
		 * Process RST Blocks Batch
		 *
		 * @since 2.9.1
		 * @return mixed
		 */
		public function process_blocks_batch() {
			...
			delete_site_option( 'rst-blocks-batch-status' );
		}
